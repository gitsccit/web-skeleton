...
class FilesApiHandler extends ApiHandler
{
     ...
     * @return array|null
     */
    public function getFile($id, $name): ?array
    {
        $cacheKey = "file_$id";
        $url = "/files/$id/$name";
    ...
    protected function getFileName($id, $name, $width = null, $height = null): string
    {
        $session = new Session();
        $file = $this->getFile($id, $name);
    ...
    public function getFileUrl($id, $name, ?int $width = null, ?int $height = null, bool $full = true): string
    {
        ...
        try {
            $fileName = $this->getFileName($id, $name, $width, $height);
